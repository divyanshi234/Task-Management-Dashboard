...
const TaskModal = ({isOpen, onClose, taskToEdit,onSave,onLogout}) => {
    ...
    useEffect(()=>{
        if(!isOpen) return;
        if(taskToEdit){
            const normalized = taskToEdit.completed === 'YES' || taskToEdit === true ? 'YES' :'NO';
            setTaskData({
                ...DEFAULT_TASK,
                title:taskToEdit.title || '',
                description: taskToEdit.description || '',
                priority:taskToEdit.priority || 'LOW',
                dueDate: taskToEdit.dueDate?.split('T') || '',
    ...
    setTaskData(prev => ({...prev, [name]:value}));
     ...
     const handleSubmit = useCallback(async (e) =>{
        ...
        try{
           ...
           console.error(error)
           setError(error.message || 'An unexpected error occur')
        }
        finally{
            setLoading(false)
        }
  ...
  return (
    ...
    <div className='fixed inset-0 backdrop-blur-sm bg-black/20 z-50 flex items-center justify-center p-4'>
        <div className=' bg-white boder border-purple-100 rounded-xl max-w-md w-full shawdow-lg relative p-6 animate-fadeIn'>
           ...
           transition-color text-gray-500 hover:text-purple-700'>
              <X className='w-5 h-5'/>
           </button>
       </div>
       ...
       <form onSubmit={handleSubmit} className='space-y-4'>
            ...
            <div>
               ...
               focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-purple-500 transition-all duration-200'>
            ...
            <div >
                <label className='felx items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
             ...
             <div className=' grid grid-cols-2 gap-4'>
                  <div>
                    <label className=' flex items-center gap-1 text-sm font-medium text-medium text-gray-700 mb-1'>
             ...
             <div>
                 <label className=' flex items-center gap-1 text-sm font-medium text-medium text-gray-700 mb-1'>
                        <CheckCircle className='w-4 h-4 text-purple-500 '/>Due Date
                    </label>
