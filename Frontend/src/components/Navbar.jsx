    ...
    const navigate= useNavigate();
     ...
     useEffect(() => {
      const handleClickOutside =(event) =>{
         if(menuref.current && !menuref.current.container(event.target)){
            setMenuOpen(false)
         }
      }
              ...
              onClick={()=> navigate('/')}>
               {/* LOGO */}
                       <div className='relative w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-br 
                        from-fuchsia-500 via-purple-500 to-indigo-500 shawdow-lg group-hover:shawdow-purple-300/50 
                         group-hover:scale-105 transition-all duration-300'>
                          <Zap className='w-6 h-6 text-white'/>
                           <div className='absolute -bottom-1 -middle-1 w-3 h-3 bg-red rounded-full shadow-md animate-ping'></div>
                       </div>
                       {/* BRAND NAME */}
                       <span className='text-2xl mt-4 font-extrabold bg-gradient-to-r from-fuchsia-500 via-purple-500 to-indigo-500 bg-clip-text text-transparent tracking-wide'>
                         TaskZone
                       </span>
                </div>
                {/* RIGHT SIDE */}
                    ...
                    <button className='p-2 text-gray-600 hover:text-purple-500 transition-colors duration-300 hover:bg-pink-50 rounded-full'
                     onClick={()=>navigate('/profile')}>
                      ...
                      <div ref={menuref} className='relative'>
                        ...
                        py-2 rounded-full cursor-pointer hover:bg-purple-50 transition-colors duration-300 border border-transparent hover:border-purple-200'>
                           ...
                           {menuOpen && (
                            <ul className='absolute top-14 right-0 w-56 bg-white rounded-2xl shawdow-xl
                            border border-purple-100 z-50 overflow-hidden animate-fadeIn'>
                                <li className='p-2'>
                                     ...
                                     className='w-full px-4 py-2.5 text-left hover:bg-purple-50 text-sm text-gray-700 transition-color flex items-center
                                     gap-2 group' role='menuitem'>
